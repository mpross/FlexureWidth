function out=findFlexure(filename)
% Usage findFlexure('fileName') [' required around the file name], outputs thickness+-error.
% Finds thinnest section of flexure scan and only requires that the flexure is running roughly top
...
%     errorbar(sliceCenters(:,2), sliceCenters(:,3), sliceCenters(:,4))
    ...
    out=[num2str(min(sliceCenters(:,3))),'+-',num2str(sliceCenters(find(min(sliceCenters(:,3))),4)),' mm'];
end
